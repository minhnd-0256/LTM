...
bool playClick(CurrentRoom *, char *message);
void onCancelBtnClick(CurrentRoom *);
void processChat(char *mess, char *user);
...
bool playClick(CurrentRoom *room, char *message)
{
...
void processChat(char *mess, char *user)
{
    ...
    strcpy(mess, "");
    Res res;
    int bytes = recvResponse(clientfd, &res);
    strcpy(mess, res.data.chat.message);
    strcpy(user, res.data.chat.username);
}
bool processPlayGame(CurrentRoom *room, char *mess)
{
